...
func (h *PublicHandler) shortcut(w http.ResponseWriter, name string) {
	...
	var filename string
	switch name {
	case "TailBlink-Send.shortcut":
		content = h.assets.Send
		filename = "TailBlink：傳送.shortcut"
	case "TailBlink-Pull.shortcut":
		content = h.assets.Pull
		filename = "TailBlink：取回.shortcut"
	default:
		h.expired(w)
		return
	}
...
<main class="shell">
  <section class="glass hero">
    <div class="brand"><span class="mark">T</span><span>TailBlink</span></div>
    <p class="eyebrow">與 {{.DeviceName}} 配對</p>
    <h1>跟著做，就設定完成。</h1>
    <p class="lead">依序安裝兩支捷徑並複製配對資料，再執行「TailBlink：取回」完成配對並取得電腦文字。若先執行「傳送」，本次只完成配對；請再複製要傳送的文字並執行一次。</p>
    {{if .AssetsReady}}
    <div class="stack">
      <a class="button secondary" href="{{.Nonce}}/TailBlink-Send.shortcut">① 安裝「TailBlink：傳送」</a>
      <a class="button secondary" href="{{.Nonce}}/TailBlink-Pull.shortcut">② 安裝「TailBlink：取回」</a>
    ...
    {{end}}
    <p class="foot">舊版使用者請安裝兩支新版捷徑；同名時選擇取代。之後請執行中文名稱的新版，避免誤開 TailBlink-Send 2 等舊副本。此頁於 {{.ExpiresAt}} 失效。</p>
  </section>
</main>
...
if (button) button.addEventListener('click', async () => {
  ...
  try {
    ...
    result.textContent = '已複製。現在執行「TailBlink：取回」，或從分享選單執行「TailBlink：傳送」。';
...
<main class="shell wide">
  <section class="glass dashboard">
    <header><div class="brand"><span class="mark">T</span><span>TailBlink</span></div><span class="badge">{{.AgentVersion}}</span></header>
    {{if .Configure}}
    ...
    {{else if eq .Mode "simple"}}
      ...
      {{if .PairingURL}}<div class="qr-wrap"><img class="qr" src="{{.QRCode}}" alt="TailBlink 簡易連線配對 QR"><span>此 QR 於 {{.ExpiresAtText}} 失效</span></div>{{end}}
    {{else if .PairingURL}}
    <div class="grid">
      <div class="copy">
      ...
      <div class="qr-wrap"><img class="qr" src="{{.QRCode}}" alt="TailBlink 限時配對 QR"><span>使用 iPhone 相機掃描</span></div>
    </div>
    {{else}}
...
const pageShellStart = `<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>TailBlink</title><style>
