...
spec = importlib.util.spec_from_file_location('tailblink_package', Path(__file__).resolve().parents[1] / 'package.py')
...
class PackageTests(unittest.TestCase):
    def test_linux_archive_permissions_and_line_endings(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            package.write_text(root / 'CLOUDFLARED.txt', 'tailblink-cloudflared-0123456789ab\n')
            (root / 'tailblink').write_bytes(b'ELF')
            (root / 'install.sh').write_bytes(b'#!/bin/bash\n')
            self.assertEqual((root / 'CLOUDFLARED.txt').read_bytes(), b'tailblink-cloudflared-0123456789ab\n')
            stream = io.BytesIO()
    ...
    def test_linux_archive_rejects_crlf(self):
        stream = io.BytesIO()
        with tarfile.open(fileobj=stream, mode='w') as archive:
            data = b'tailblink-cloudflared-0123456789ab\r\n'
