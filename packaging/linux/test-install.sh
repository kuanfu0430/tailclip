...
set -Eeuo pipefail
[[ ${TAILBLINK_INSTALL_TEST:-} == 1 ]] || exit 2
source /etc/os-release
[[ $ID:$VERSION_ID == debian:13 ]] || exit 2
root=$(mktemp -d /home/tailblink-install-test.XXXXXX)
trap 'rm -rf -- "$root"' EXIT
mkdir -p "$root/mock" "$root/package"
cp "$(dirname "$0")/install.sh" "$(dirname "$0")/tailblink.service" "$(dirname "$0")/uninstall.sh" "$root/package/"
...
printf '#!/bin/sh\nprintf '\''{"service":"tailblink","ok":true}\\n'\''\n' > "$root/mock/curl"
printf '#!/bin/sh\nexit 99\n' > "$root/mock/sudo"
printf '#!/bin/sh\nprintf "binary:%%s\\n" "$*" >> "$TEST_LOG"\n' > "$root/package/tailblink"
printf 'tailblink-cloudflared-123456789abc\n' > "$root/package/CLOUDFLARED.txt"
printf '#!/bin/sh\nexit 0\n' > "$root/package/tailblink-cloudflared-123456789abc"
chmod +x "$root/mock/"* "$root/package/tailblink" "$root/package/tailblink-cloudflared-123456789abc"
export PATH="$root/mock:/usr/bin:/bin"
command -v tailscale && exit 3
bash "$root/package/install.sh"
test -x "$root/.local/bin/tailblink"
test -x "$root/.local/bin/tailblink-cloudflared-123456789abc"
...
test ! -e "$root/.local/bin/tailblink"
test ! -e "$root/.local/bin/tailblink-cloudflared-123456789abc"
printf 'y\n' | bash "$root/package/uninstall.sh"
