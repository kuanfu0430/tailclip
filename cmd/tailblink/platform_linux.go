...
func prepareTailnet(ctx context.Context) error {
	...
	return fmt.Errorf("尚未設定 HTTPS。完整名稱 %s 將出現在公開憑證紀錄；同意後請在終端執行 sudo ~/.local/bin/tailblink serve-install，再回來選擇 Tailscale。", status.Self.DNSName)
}
...
func showFatal(err error) { _, _ = fmt.Fprintln(os.Stderr, "TailBlink:", err) }
