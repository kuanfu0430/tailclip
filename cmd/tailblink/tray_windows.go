...
func (tray *windowsTray) run(ctx context.Context, ready chan<- error) error {
	...
	if instance == 0 {
		err := windowsCallError("無法取得 TailBlink 程序模組", callErr)
		ready <- err
		return err
	}
	className, err := windows.UTF16PtrFromString(fmt.Sprintf("TailBlinkTrayWindow.%d", os.Getpid()))
	if err != nil {
		ready <- err
		return err
	}
	windowName, err := windows.UTF16PtrFromString("TailBlink")
	if err != nil {
		ready <- err
		return err
	}
	...
	if atom == 0 {
		err := windowsCallError("無法註冊 TailBlink 通知區視窗", callErr)
		ready <- err
		return err
	}
	...
	if window == 0 {
		err := windowsCallError("無法建立 TailBlink 通知區視窗", callErr)
		ready <- err
		return err
	}
	...
	if icon == 0 {
		err := windowsCallError("無法載入 TailBlink 通知區圖示", callErr)
		ready <- err
		return err
	}
	tray.icon = icon
	...
	for {
		...
		switch int32(result) {
		case -1:
			return windowsCallError("TailBlink 通知區訊息迴圈失敗", callErr)
		case 0:
			return nil
		default:
...
func (tray *windowsTray) addIcon() error {
	...
	if window == 0 {
		return errors.New("TailBlink 通知區視窗尚未就緒")
	}
	...
	tip, _ := windows.UTF16FromString("TailBlink（按兩下開啟）")
	copy(data.Tip[:], tip)
	result, _, callErr := procShellNotifyIconW.Call(nimAdd, uintptr(unsafe.Pointer(&data)))
	if result == 0 {
		return windowsCallError("無法加入 TailBlink 通知區圖示", callErr)
	}
	return nil
}
...
func (tray *windowsTray) showMenu() {
	...
	if menu == 0 {
		showFatal(windowsCallError("無法建立 TailBlink 選單", callErr))
		return
	}
	...
	if err := appendTrayMenu(menu, mfString, trayMenuExit, "結束 TailBlink"); err != nil {
		showFatal(err)
		return
	}
	var point trayPoint
...
func appendTrayMenu(menu, flags, id uintptr, label string) error {
	...
	if result == 0 {
		return windowsCallError("無法建立 TailBlink 選單項目", callErr)
	}
	return nil
}
