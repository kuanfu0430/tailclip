...
func TestNewInstallationDoesNotRequireTailscale(t *testing.T) {
	...
	if err := installer.finish(context.Background(), "TailBlink.exe"); err != nil {
		t.Fatal(err)
	}
...
func TestWindowsInstallerCopiesThenCompletesSynchronously(t *testing.T) {
	root := t.TempDir()
	source := filepath.Join(root, "download", "TailBlink.exe")
	target := filepath.Join(root, "local", "TailBlink", "TailBlink.exe")
...
func TestWindowsInstallerReusesExistingServe(t *testing.T) {
	...
	if err := installer.finish(context.Background(), `C:\TailBlink\TailBlink.exe`); err != nil {
		t.Fatal(err)
	}
...
func TestWindowsInstallerDoesNotElevateKnownStateError(t *testing.T) {
	...
	err := installer.finish(context.Background(), `C:\TailBlink\TailBlink.exe`)
...
func TestAutostartCommandQuotesExecutablePath(t *testing.T) {
	got := autostartCommand(`C:\Users\Tester Name\AppData\Local\TailBlink\TailBlink.exe`)
	want := `"C:\Users\Tester Name\AppData\Local\TailBlink\TailBlink.exe" agent`
	if got != want {
...
func TestWindowsPackagedInstallIntegration(t *testing.T) {
	directory := os.Getenv("TAILBLINK_WINDOWS_PACKAGE_DIR")
	if directory == "" {
		t.Skip("需要 TAILBLINK_WINDOWS_PACKAGE_DIR 指向完整發行包")
	}
	source := filepath.Join(directory, "TailBlink.exe")
	target := filepath.Join(t.TempDir(), "中文 使用者", "TailBlink.exe")
