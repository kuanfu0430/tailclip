...
func (installer windowsInstaller) prepare(ctx context.Context, executable string) error {
	...
	if !accepted {
		return errors.New("已取消 HTTPS 設定；再次雙擊 TailBlink 即可繼續")
	}
...
func uninstallAction(ctx context.Context, executable string) error {
	confirmed, err := message("解除安裝 TailBlink", "將移除 TailBlink 的自啟、/tailblink Serve path、設定與日誌。\n\n不會修改 Tailscale 的其他 Serve 設定。確定繼續嗎？", mbYesNo|mbIconInformation)
	if err != nil {
		return err
	}
	if confirmed != idYes {
	...
	_, _ = message("TailBlink", "解除安裝已完成。TailBlink 檔案會在視窗關閉後移除。", mbOK|mbIconInformation)
	return nil
}
...
func windowsInstallPath() (string, error) {
	...
	return filepath.Join(base, "TailBlink", "TailBlink.exe"), nil
}
...
func copyExecutable(source, target string) error {
	...
	if err != nil {
		return fmt.Errorf("無法讀取 TailBlink.exe: %w", err)
	}
	defer input.Close()
	...
	if _, err := io.Copy(output, input); err != nil {
		...
		return fmt.Errorf("無法複製 TailBlink.exe: %w", err)
	}
	...
	if _, statErr := os.Stat(target); statErr == nil {
		...
		if err := os.Rename(target, backup); err != nil {
			_ = os.Remove(temporary)
			return fmt.Errorf("無法更新安裝檔；請先結束正在執行的 TailBlink 後再試: %w", err)
		}
	}
...
func removeExecutableBackup(target string) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("新版 TailBlink 已啟動，但無法移除舊執行檔 %s: %w", backup, err)
		}
...
func confirmHTTPS(dnsName string) (bool, error) {
	text := "TailBlink 將為下列私有 tailnet 位址啟用 HTTPS：\n\n" + dnsName + "\n\n完整名稱會出現在公開的 Certificate Transparency 紀錄，但服務仍只在你的 tailnet 內可達。是否繼續？"
	result, err := message("TailBlink HTTPS 設定", text, mbYesNo|mbIconInformation)
...
func showFatal(err error) { _, _ = message("TailBlink", err.Error(), mbOK|mbIconError) }
